...
class App extends Component {
  ...
  render() {
    ...
    const searchResults = destinationsList.filter(eachTemple =>
      eachTemple.name.includes(searchInput),
    )
    return (
      <div className="card-container">
        <h1 className="heading">Destination Search</h1>
        <input type="search" onClick={this.onSearchInput} value={searchInput} />
        <ul className="list-container">
          {searchResults.map(eachTemple => (
            <DestinationSearch
              destinationDetails={eachTemple}
              key={eachTemple.id}
            />
          ))}
        </ul>
